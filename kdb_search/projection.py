...
class ProjectedEntity:
    ...
    **The body is delivered whole (D-123-C).** There was a `truncated` flag here,
    set by a word cap and a per-entity byte ceiling. It was computed on every
    projection and read by nothing — `render_fat_block` never emitted it — while
    the prose it justified told the model an excerpt's silence was weak evidence,
    which was false for 161 of 163 fixture entities. Deleting the mechanism
    deleted the defect.
    """
...
def stream_contribution_bytes(projected: ProjectedEntity) -> int:
    ...
    **This is the fill's unit of cost (D-123-B).** It was the quantity the retired
    ...
    The conservative reading is kept deliberately. `"\\n".join()` over n blocks
    costs n-1 separators, not n, so summing this over a pool overstates the real
    request by exactly one byte — the fill stops one byte early rather than one
    byte late, and the pre-flight that follows re-measures the true rendering.
    """
...
def project_entity(entity: SpaceEntity, *, body_reader: BodyReader) -> ProjectedEntity:
    """Read one entity's body and project it to evidence, **verbatim** (D-123-C).
    Callers never read bodies — projection runs inside search (spec §1.1).

    Nothing is cut here any more. Sizing is the fill's job, one level up: it can
    decline to include an entity, which is honest, where this function could only
    corrupt one, which was not.
    """
    try:
