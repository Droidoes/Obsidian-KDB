...
`from kdb_compiler.x import Y` inside `graphdb_kdb/`).
"""
...
class ProducerAdapter(Protocol):
    ...
    `graphdb_kdb/adapters/<producer>_runs.py` per D-B1 naming convention.
    """
    ...
    def apply(
        ...
        For Obsidian v1: delegates to `graphdb_kdb.ingestor.apply_compile_result`.
    ...
    def sync_current_run(
        ...
        `graphdb_kdb.GraphDB` or `apply_compile_result` directly (D-S0).
        """
        ...
