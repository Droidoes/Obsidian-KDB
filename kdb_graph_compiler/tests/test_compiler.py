"""Tests for kdb_graph_compiler — per-source compile orchestration (mocked seam).
...
All tests use `monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", fake)`
...
def test_compile_meta_attempts_reflects_reprompt_count(
    ...
    This lets the kdb_graph_orchestrator recorder surface the retry count in console.log.
    """
    vault = _write_vault(tmp_path)
    _write_raw(vault, SOURCE_A, "alpha body")
    state_root = vault / "KDB" / "state"
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", schema_bad_then_good)
...
def test_failure_triplet_prompt_build_populates(
    ...
    monkeypatch.setattr(
        "kdb_graph_compiler.compiler.prompt_builder.build_prompt", boom
    )
...
def test_source_text_for_returns_tuple_with_frontmatter(tmp_path: Path) -> None:
    ...
    from kdb_graph_compiler.compiler import source_text_for
...
def test_response_schema_omits_page_domain():
    ...
    schema = json.loads(pathlib.Path(
        "kdb_graph_compiler/schemas/compiled_source_response.schema.json").read_text())
...
def test_rung1_escape_recovers_latex_on_attempt_1(
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", fake)
...
def test_collision_falls_through_to_quarantine(
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", always_collision)
...
def test_non_slug_schema_error_unchanged(
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", always_bad)
...
def test_irreparable_json_quarantines(
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", always_broken)
