...
The gate expression lives in kdb_graph_compiler/compiler.py's `finally` block:
...
This test drives the *real* kdb_graph_compiler path (not a unit-test of the
...
into common/llm_telemetry (generic) + kdb_graph_compiler/resp_summary (kdb_graph_compiler-only).
"""
