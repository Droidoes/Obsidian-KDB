"""Tests for kdb_graph_compiler.kpi.graph.compute_graph (#109).
...
from kdb_graph_compiler.kpi.graph import compute_graph, _largest_component_fraction
...
def graph_dir(tmp_path: Path) -> Path:
    ...
    not importable here as kdb_graph_compiler/tests has its own conftest scope)."""
...
def test_no_finalize_branch_skips_finalized_reads(graph_dir, monkeypatch):
    ...
    monkeypatch.setattr("kdb_graph_compiler.kpi.graph.queries.active_canonical_entity_slugs", boom)
    monkeypatch.setattr("kdb_graph_compiler.kpi.graph.queries.total_source_count", boom)
    monkeypatch.setattr("kdb_graph_compiler.kpi.graph.queries.links_to_edges", boom)
    monkeypatch.setattr("kdb_graph_compiler.kpi.graph.queries.resolve_to_canonical_slugs", boom)
...
def test_no_resolver_call_in_the_context_evidence_path(graph_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "kdb_graph_compiler.kpi.graph.queries.resolve_to_canonical_slugs", boom_resolve)
