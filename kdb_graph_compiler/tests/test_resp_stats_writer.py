...
NOTE (Phase B split): Functions now live in:
  ...
  - kdb_graph_compiler.resp_summary: build_parsed_summary
...
def test_build_resp_stats_stores_none_when_parsed_summary_omitted(tmp_path: Path) -> None:
    ...
    is covered in kdb_graph_compiler/tests/test_parsed_summary_gate.py."""
    ctx = _ctx(tmp_path)
    record = resp_stats_writer.build_resp_stats(
        ctx=ctx, source_id="KDB/raw/foo.md",
        prompt=_FakePrompt(system="S", user="U"),
...
def test_failure_triplet_populated_from_telemetry(tmp_path: Path) -> None:
    ...
    kdb_graph_compiler import into the resp_stats_writer test surface."""
