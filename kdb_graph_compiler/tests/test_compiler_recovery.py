...
Contract under test (spec §3.3):
  * Recovery runs FIRST on every model response (unwrap + strict-eval +
    5-step selection ladder in kdb_graph_compiler.response_recovery). Callers branch
...
Harness mirrors kdb_graph_compiler/tests/test_compiler.py (imported helpers + the
same monkeypatched ``kdb_graph_compiler.compiler.call_model_with_retry`` seam).
"""
...
from kdb_graph_compiler.tests.test_compiler import (
    _ctx,
    _failure_fields,
    _good_response,
    _job,
