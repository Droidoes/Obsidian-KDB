"""kdb_graph_compiler.kpi.score — Borda scoring mechanism for cross-model benchmark ranking.
...
Provides:
  borda_normalize(...)  — average-rank Borda normalization (§7 spec). Originally
                          ...
                          kdb_graph_compiler must NOT import tools; tools MAY import kdb_graph_compiler.

  borda_score(...)      — composite Borda scorer over the per-model "scored" KPI
                          dicts produced by kdb_graph_compiler.kpi.processing / graph.
...
KPI_LOWER_IS_BETTER: dict[str, bool] = {
    ...
    # combined graph score; no single one suffices (see kdb_graph_compiler.kpi.graph).
    "entity_reuse":         False,
...
# violate the B.3 kdb_graph_compiler→tools ban. At runtime the annotation is never
