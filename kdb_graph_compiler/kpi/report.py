"""kdb_graph_compiler.kpi.report — render a single-run measurements payload as Markdown.

`render_report(payload)` turns the `measurements.json` dict (header + processing
+ graph, the shape assembled by kdb_graph_orchestrator.emit_kpis) into a human-readable
