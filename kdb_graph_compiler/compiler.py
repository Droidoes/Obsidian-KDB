"""kdb_graph_compiler — per-source LLM compile orchestration.
...
monkeypatch `kdb_graph_compiler.compiler.call_model_with_retry` as a clean
...
from kdb_graph_compiler import (
    canonicalize,
    prompt_builder,
    proposal_bridge,
...
from kdb_graph_compiler.canonicalize import AliasLedger
from kdb_graph_compiler.context_loader import (
    _DEFAULT_PAGE_CAP,
...
from kdb_graph_compiler.search_adapter import run_pass15
...
def source_text_for(job: CompileJob) -> tuple[SourceFrontmatter | None, str]:
    ...
    Task #91: prefers the kdb_graph_orchestrator's in-memory (source_text, frontmatter)
...
def compile_one(
    ...
    *,
    ...
    sufficiency; the normalization bridge (kdb_graph_compiler.proposal_bridge) then
    ...
    # Task #91: derive from source_id, not abs_path — the in-memory kdb_graph_orchestrator
    ...
    try:
        ...
        state["compiled_source"] = CompiledSource(
            ...
            compile_meta=CompileMeta(
                ...
                # meaningful for the kdb_graph_orchestrator recorder (Fix 3b).
...
def compile_source(
    ...
    *,
    ...
    `cr`; the kdb_graph_orchestrator owns stage-8
    ...
    kdb_graph_orchestrator routes case-(a) (D-91-13) uniformly without string-parsing.

    #123 P3a.2b (§4.4): `selector` is the run-level pass-1.5 seat ModelSpec
    (the kdb_graph_orchestrator resolves and validates it once per run); selector=None
