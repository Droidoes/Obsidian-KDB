...
- **UPDATE 2026-08-08 (evening):** (a) RESOLVED — both `Sample-Prompt-Pass-*.md` moved to `Projects/Obsidian-KDB/prompt/` + `"prompt/"` added to pipeline `excludes`; erased via the DELETED path. **Counting nuance, now exercised for the first time:** deletion erases pages/entities but retains the Source node as a `status: deleted` tombstone — so `graphdb-kdb stats`'s raw `sources` counts tombstones; the correct consistency check is **graph sources `WHERE status='active'` == manifest `in_graph_db`** (holds: 1,148 = 1,148). (b) RESOLVED 2026-08-08 ~16:37 — the Buffett doc's `quotes` failure was deterministic **8/8** (gpt-5.4-mini ×6 incl. one owner edit, deepseek-v4-flash ×2) until the owner's v3 edit reframed the document's nature; it then classified `transcript-video` / `value-investing` and committed clean. **The vault is now fully settled: 1,149 in_graph_db + 451 noise + 0 error states.** Residual (non-urgent): a pure quote-collection doc will still trip the missing `quotes` contract — the NW-7 prompt-guidance line ("quotes is a domain, never a source_type; classify by publication form") is the filed fix if one ever appears.
