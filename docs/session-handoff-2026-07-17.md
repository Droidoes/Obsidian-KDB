...
## OPEN — pick up here
- [ ] **Vault-in-place ingestion brainstorm** — now unambiguously next. Absorb the two new facts (OneDrive mount; OneNote swamp) alongside §7 of the state doc. First concrete move unchanged: X6/`force_noise` exclusion spot-check on real notes.
- [ ] **Joseph's call pending:** delete `~/Droidoes/GraphDB-KDB` (safe — nothing references it post-3.2).
- [ ] Open question for the brainstorm: where should the comprehensive graph live, given OneDrive? (D35's corruption concern vs the vault-derived default.)

## Housekeeping
- **Committed + pushed:** 3 commits (code hygiene / docs sync / review + handoffs) + tag `v0.5.7` on `main`. `AGENTS.md`/`QWEN.md` updated on disk but gitignored by design.
