...
| D47 | 2026-05-16 (Task #70) | **`KDB_CONTEXT_SOURCE` default stays `manifest`** — cold-start blocker resolved (#71, `8f6ae11`). Graph now exceeds manifest on cold-start (23 vs 8 pages). Default flip to `graphdb` is a D49 candidate. | Original blocker: new sources with no SUPPORTS edges got less context from graph than manifest. Resolved by title-phrase matching + conditional 2-hop T3 (see §8.7). Remaining gate for default flip: full regression pass across all sources. |
