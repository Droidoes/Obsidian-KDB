...
| D47 | 2026-05-16 (Task #70) | **`KDB_CONTEXT_SOURCE` default stays `manifest`** until cold-start widening rule lands (#71). GraphDB context is opt-in only via env var. | Cold-start gap: new sources with no SUPPORTS edges get significantly less context from graph than from manifest. Flipping default prematurely would degrade first-compile quality. |
