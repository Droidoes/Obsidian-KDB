...
| 35 | in-progress   | `kdb-benchmark --verbose` per-measure scoring trace            | Surfaced 2026-05-07 alongside #34. After #34 fixed the wiring, #35 makes the scoring derivation legible to the user. `score_run` + `score_runs` accept `verbose: bool = False` (no behavior change when False). When True, each S0/S1/S2/S3 + M1..M7 + diagnostics line is printed with numerator, denominator, rate, weight; evidence snippets surface S0 cause (parse / schema / hard-zero per record), M1 target_set size, M6 cost-vs-words, M7 latency-vs-words, and Borda raw→ranked → final_score derivation. CLI gains `--verbose` / `-v` flag |
