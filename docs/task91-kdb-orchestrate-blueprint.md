...
| **D-91-15** | **AMENDS D-91-13 → graph-sync-first commit ordering (β).** Per-source commit order is **apply-wiki → graph-sync (Kuzu txn) → [on success] manifest + sidecar** (was manifest-first). A graph-sync failure rolls back cleanly → manifest never written → **case-(a) self-heal** on re-run (`apply_compile_result` is per-source idempotent). **Case-(b) ("manifest committed, graph stale → manual rebuild") is ELIMINATED.** Strengthens D-91-13's replayable-authority intent: `sidecar exists ⇒ manifest written ⇒ graph consistent`. Residual: crash between graph-sync COMMIT and sidecar-write + graph loss = bounded one-source double-fault (same class accepted for journaling). Also: cross-source `LINKS_TO` wiring deferred to a finalize batch pass (`wire_links=False` per-source + standalone `wire_links()` at finalize) so live≡replay holds (per-source graph-sync can't see later sources' entities). | 2026-05-29 | **5/5 panel UNANIMOUS for β + Joseph ratified the D-91-13 revision.** Justified by conditions post-dating D-91-13: per-source (not batch) graph-sync, the single read-write connection, and *verified* clean Kuzu rollback + per-source idempotency. C1 (cross-source LINKS_TO) caught by Gemini (1/5) in the Plan 5+6 panel; defer-to-finalize chosen 4/5 over stub-upsert. Synthesis: `docs/task91-c1-ford-synthesis.md`. |
