...
| **D51** | #73 closure | **GraphDB is the live ontology authority; `state/runs/` sidecars are reconstruction material, not the primary data flow.** Layer model: `raw/` = source corpus; `GraphDB-KDB/` = live ontology (primary); `state/runs/` = audit log + recovery; `manifest.json` = source-file lifecycle metadata (source-state); `wiki/` = markdown rendering. |
