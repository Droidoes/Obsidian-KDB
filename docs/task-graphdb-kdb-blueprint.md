...
|---|---|---|
| **Q3** | **Run-journal replay contract** — does the v2 run journal carry both (a) eligibility fields (`success`, `dry_run`) and (b) replay payload (`compile_result` + `last_scan`)? Determines whether `graphdb-kdb rebuild` can apply the D39 eligibility filter retroactively. | **Owned by sub-task #63.0, which gates all other sub-tasks.** Per the four-outcome decision matrix in §11: (a) both present → confirm and proceed; (b) eligibility fields missing → add to write-side BEFORE #63.1; (c) payload missing → write-side embed OR sidecar archive; (d) historical unrecoverable → downgrade D39 to prospective-only + one-off backfill. Outcome recorded in D39 rationale and CODEBASE_OVERVIEW.md §8 once Task #63 closes. |
