...
On **selector failure** — transport error, timeout, or any §2.3 response-level class (`unparseable_response`, `structurally_unusable_response`, `all_entries_dropped`; model-correctable/transient classes per D-119's "retries only for model-correctable failure classes"): **retry the LLM call**, bounded budget (2 attempts total, the #104/#106 precedent). `budget_exceeded` (§7.2 R2) is **not** a retry class — it is deterministic, fails before any API call, and is reported, not retried. **`budget_estimation_miss` (D7) is likewise never retried:** the provider's context-length rejection means the estimate under-called a rendered request; the same request fails identically. It is typed (`status: selector_failure`, `failure_class: budget_estimation_miss`), the call is recorded as attempted (spend possible), and its rate is watched per model (promotion to gate = later decision — codex L1 option 2).
...
Counts — eligible-space size, stage-1 retained, stage-2 hydrated, stage-2 title-only, returned entries, **valid_entry_yield** (`None` when returned = 0, §2.3), **per-class attempted violations** (§2.3), `all_entries_dropped` occurrences, `unattributed_hit_count`, `query_truncated` occurrences (D6), `search_artifact_write_failures` (warn-only envelope sink — codex minor), retry attempts, `selector_failure` class — `status`, `execution`, per-stage model route + stamps, per-stage latency + cost, **pre-flight budget record (`budget_estimate_tokens`, `selector_window`, `headroom_factor`, routing outcome — §7.2 R2)**, **thin/fat concordance (fat top-10 ∩ thin top-20 — §8.3)**, search_snapshot_hash, artifact ref. Threads into the #122 record (§5.1) and the run's KPI emission; **selector-quality fields (yield, per-class violation rates, retry rate, failure rate) are watched KPIs per model — Joseph's leaderboard counter (R1 §3.2), promotion to scored is a later, separate decision.**
...
| vault (~1,706 notes) | **~9,600** (5.6/src) | **~3,000** (31% share) | **~211k tokens** (~845 kB measured projection) | **~18k tokens expected** (100 × 719 B); **statically guaranteed to fit the pool** — ≤ ~257k tokens absolute worst case + 4k output < 320k smallest budget (M=100, D7) |
