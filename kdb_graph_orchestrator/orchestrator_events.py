...
Task #96 B1-B2 foundation: the kdb_graph_orchestrator needs machine-readable run events
...
class EventRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        if log_level not in LOG_LEVELS:
            raise ValueError(f"unknown kdb_graph_orchestrator log level: {log_level}")
        self.run_id = run_id
    ...
    def _write_progress(self, event: OrchestratorEvent, et: str) -> None:
        ...
        if et == "source_started":
            ...
            # The kdb_graph_orchestrator threads attempts into event.context when the compile
...
def check_orchestrator_invariant(
    ...
    *,
    ...
    """Always-on production invariant check for kdb_graph_orchestrator contracts."""
    if condition:
        return
    recorder.record(
        stage=stage,
