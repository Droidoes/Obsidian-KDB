...
pattern). Run: python -m pytest kdb_graph_orchestrator/tests/test_kdb_orchestrate.py -m "not live"
"""
import hashlib
import json
import shutil
...
from kdb_graph_orchestrator import kdb_orchestrate
import kdb_graph_orchestrator.emit_kpis as _emit_kpis_mod
...
def _capture_pass_leaves(monkeypatch, captured: dict) -> None:
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", pass2_leaf)
...
def test_finalize_runs_after_later_source_quarantine_and_wires_committed_links(
    ...
    monkeypatch.setattr("kdb_graph_compiler.compiler.call_model_with_retry", model)
...
def test_emit_kpis_writes_measurements_json(tmp_path, monkeypatch):
    ...
    from kdb_graph_compiler.prompt_builder import load_system_prompt
...
# ============================================================================
# #132: replay journal + sidecar archival (D39 for the kdb_graph_orchestrator era)
# ============================================================================
...
def test_journal_archive_failure_sets_replayable_false(tmp_path, monkeypatch):
    ...
    from kdb_graph_orchestrator import journal_writer
...
def test_run_journals_rebuild_identical_graph_e2e(tmp_path, monkeypatch):
    """#132 E2E (D39 restored): three kdb_graph_orchestrator runs — cold (3 sources),
    ...
    monkeypatch.setattr(
        "kdb_graph_compiler.search_adapter.call_model",
        fakes.FakeSelector(*[
