...
feeder → ingestion (Pass-1 enrich) → kdb_graph_compiler (Pass-2 compile_source) → GraphDB,
...
from kdb_graph_orchestrator import journal_writer, manifest_writer
...
from kdb_graph_compiler.compiler import compile_source
from ingestion.enrich.enrich import enrich_one
from ingestion.kdb_scan import scan_scope
from kdb_graph_orchestrator.orchestrator_events import (
    EventRecorder,
...
from kdb_graph_orchestrator.emit_kpis import maybe_emit_kpis
...
from kdb_graph_compiler.prompt_builder import PASS2_PROMPT_VERSION, load_system_prompt
...
def run(
    *, pipeline_id: str, vault_root: Path, state_root: Path, graph_path: Path,
    ...
    recorder.record(
        ...
        message="kdb_graph_orchestrator run started",
    ...
    if dry_run:
        ...
        recorder.record(
            ...
            message="kdb_graph_orchestrator run finished",
    ...
    try:
        ...
        recorder.record(
            ...
            message="unexpected kdb_graph_orchestrator exception",
            exception_type=type(e).__name__, error=str(e))
        raise
    finally:
        finished_at = now_iso()
        ...
        # re-derive an kdb_graph_orchestrator-era graph. Partial on abort/crash
